...
Perl_pad_leavemy(pTHX)
{
	    ...
	    && COP_SEQ_RANGE_HIGH(sv) == PERL_PADSEQ_INTRO)
	{
	    ...
	    if (!SvPAD_STATE(sv) && !SvPAD_OUR(sv) && *SvPVX(sv) == '&'
	     && SvCUR(sv) > 1) {
...
S_cv_clone_pad(pTHX_ CV *proto, CV *cv, CV *outside)
{
	...
	if (namesv && namesv != &PL_sv_undef) { /* lexical */
	    ...
	    if (!sv) {
		    ...
		    else if (SvCUR(namesv) > 1 && !SvPAD_OUR(namesv)) {
			/* my sub */
...
S_cv_clone(pTHX_ CV *proto, CV *cv, CV *outside)
{
    ...
    if (CvPADLIST(proto)) S_cv_clone_pad(proto, cv, outside);

    DEBUG_Xv(
	PerlIO_printf(Perl_debug_log, "\nPad CV clone\n");
	cv_dump(CvOUTSIDE(cv), "Outside");
	cv_dump(proto,	 "Proto");
	cv_dump(cv,	 "To");
    );
